...
import { Button, TextField, Box, Typography, Dialog, DialogTitle, DialogContent, DialogActions, Stack, MenuItem, Snackbar, Alert, Tabs, Tab, Paper, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, Grid, Chip, IconButton, FormControl, InputLabel, Select } from '@mui/material';
import { getGoodsIssues, createGoodsIssue } from '../api/goodsIssues';
...
export default function GoodsIssueList() {
  ...
  const loadData = async () => {
    try {
      const [issues, customersData, productsData] = await Promise.all([
        getGoodsIssues(),
        getCustomers(),
        getProducts()
      ]);
      
      setRows(issues.map(issue => ({
        ...issue,
        id: issue.id,
        total_items: issue.items?.length || 0,
        total_amount: issue.items?.reduce((sum, item) => sum + (item.price * item.quantity), 0) || 0,
  ...
  const handleRowClick = (params) => {
    const issue = rows.find(r => r.id === params.id);
    if (issue) {
      setIsEditing(true);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (isEditing) {
        ...
        console.log('Updating issue:', editingId, issueData);
      } else {
        const created = await createGoodsIssue(issueData);
      ...
      if (error.response && error.response.data && error.response.data.error === 'Insufficient stock') {
        setError(error.response.data);
      } else {
        setError({ error: 'Сталася помилка при створенні накладної.' });
      }
    }
  };
  ...
  return (
    <Box sx={{ p: 2 }}>
      <Typography variant="h5" gutterBottom>Продажі (Видаткові накладні)</Typography>
      <Box sx={{ mb: 2 }}>
      ...
      <Dialog open={open} onClose={handleClose} maxWidth="xl" fullWidth>
        <DialogTitle sx={{ bgcolor: 'primary.main', color: 'white', p: 2 }}>
          <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <Typography variant="h6">
              {isEditing ? 'Редагування' : 'Створення'} видаткової накладної {form.number} від {form.date} {form.time} ({form.operation_type})
            </Typography>
        ...
        <DialogActions sx={{ p: 2, bgcolor: 'grey.100' }}>
          ...
          <Button
            ...
            startIcon={<PrintIcon />}
            disabled={!editingId}
            onClick={() => editingId && window.open(`http://localhost:5000/api/goods_issues/${editingId}/print`, '_blank')}
          >
            Друк
          </Button>
          <Button variant="outlined">
            ...
            <Box sx={{ mt: 1 }}>
              {error.details.map((detail, index) => (
                <Typography key={index} variant="body2">
                  Товар ID {detail.product_id}: потрібно {detail.required}, доступно {detail.available}
                </Typography>
              ))}
            </Box>
          )}
